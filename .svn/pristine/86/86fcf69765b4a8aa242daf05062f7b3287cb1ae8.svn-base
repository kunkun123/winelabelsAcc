...
!function(a) {
					...
					"POST" == i && "object" == typeof n && (p = a.extend({}, p, n)), a.ajax(p).error(function() {
						...
						if ("function" == typeof h.settings.update_active)
							...
							if (q.length > 0) {
								...
								});o.closest("li").addClass("active").parents(".nav li").addClass("active open");q.closest(".sidebar[data-sidebar-scroll=true]").each(function() {
									var b = a(this);
		...
		this.startLoading = function() {
			...
			if (d && 0 != d.length) {
				if (h.settings.close_mobile_menu) try {
					...
					} catch (b) {} h.settings.close_dropdowns && a(".dropdown.open .dropdown-toggle").dropdown("toggle"), h.loadUrl(d, null, c)
			}
		}).trigger("hashchange.ace_ajax", [ !0 ]);var n = a.trim(window.location.hash);
	...
	function c(b, c) {
		...
		this.settings = a.extend({}, a.fn.ace_sidebar.defaults, c, g), this.minimized = !1, this.collapsible = !1, this.horizontal = !1, this.mobile_view = !1, this.vars = function() {
			...
			return this
				...
				if (f.length > 0) {
					d = f.attr("data-icon1"), e = f.attr("data-icon2"), "undefined" != typeof b ? b ? f.removeClass(d).addClass(e) : f.removeClass(e).addClass(d) : f.toggleClass(d).toggleClass(e);try {
		...
		this.toggleMenu = function(c, d) {
			...
			var d = !(c === !1 || d === !1);
			this.minimized ? this.$sidebar.addClass("menu-min") : this.$sidebar.removeClass("menu-min");try {
				d && ace.settings.saveState(b, "class", "menu-min", this.minimized)
			} catch (e) {} return c || (c = i()), c && h.call(c, this.minimized, d), ace.vars.old_ie && ace.helper.redraw(b), a(document).trigger("settings.ace", [ "sidebar_collapsed", this.minimized, b, d ]), this.minimized ? this.$sidebar.trigger(a.Event("collapse.ace.sidebar")) : this.$sidebar.trigger(a.Event("expand.ace.sidebar")), !0
	...
	function c(b, c) {
		...
		function k(c) {
			...
			g && (o = n, m.top += o);var t = parseInt(m.top + r),
				...
				B = q.horizontal,
				C = !1;
			B && this.parentNode == p && (v = 0, m.top += d.height(), C = !0), !C && (v = t - (y + h)) >= 0 && (v = z > v ? v : z, 0 == v && (v = 20), z - v > 10 && (v += parseInt(Math.min(25, z - v))), m.top + (n - o) > t - v && (v -= m.top + (n - o) - (t - v)), v > 0 && (c.style.top = -v + "px", g && (g.style.marginTop = -v + "px"))), 0 > v && (v = 0);var D = v > 0 && v > n - 20;
			if (D ? d.addClass("pull_up") : d.removeClass("pull_up"), B)
				if (d.parent().parent().hasClass("hover-flip")) d.addClass("hover-flip");else {
					var E = f.offset(),
						F = f.width(),
						G = u.width();
	...
	function c(b, c) {
		...
		a(window).off("resize.widget.scroll");var e = ace.vars.old_ie || ace.vars.touch;
		if (c) {
	...
	var d = function(b, d) {
		this.$box = a(b);this.reload = function() {
			var a = this.$box,
				b = !1;
	...
	} catch (c) {} a("#skin-colorpicker").on("change", function() {
		function b(b) {
