...
function iScroll (el, options) {
	...
	if (isTouch || that.options.desktopCompatibility) {
		...
		that.element.addEventListener("gesturestart", that, false);
		that.element.addEventListener("gestureend", that, false);
	}
...
function scrollbar (dir, wrapper, fade, shrink, color) {
	...
	that.wrapper.style.cssText = '-webkit-mask:-webkit-canvas(scrollbar' + that.uid + that.dir + ');position:absolute;z-index:10;pointer-events:none;overflow:hidden;opacity:1;-webkit-transition-duration:' + (fade ? '300ms' : '0') + ';-webkit-transition-delay:0;-webkit-transition-property:opacity;' +
