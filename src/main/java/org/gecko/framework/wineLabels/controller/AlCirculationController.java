 ...
 */
@Controller
@RequestMapping("/wineLabels/alcirculation")
public class AlCirculationController extends BaseController<AlCirculation> {
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_WINELABELS_AL_CIRCULATION_QUERY"})
	public Page<AlCirculation> list(Page<AlCirculation> page) {
        AlCirculation param = new AlCirculation();
        if(page.getSearch()!=null){
        param.setTemp1("%" + page.getSearch() + "%");
//param.setEndtime("%" + page.getSearch() + "%");
param.setProductnum("%" + page.getSearch() + "%");
//param.setLastupdatetime("%" + page.getSearch() + "%");
param.setTopentid("%" + page.getSearch() + "%");
param.setBelowentid("%" + page.getSearch() + "%");
param.setTemp3("%" + page.getSearch() + "%");
//param.setStarttime("%" + page.getSearch() + "%");
...
param.setCreator("%" + page.getSearch() + "%");
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_WINELABELS_AL_CIRCULATION_VIEW"})
	public R get(@PathVariable String id) {
		AlCirculation alcirculation = service.get(id);
		return R.ok().put(alcirculation);
	}

    @RequestMapping("/edit")
    @ResponseBody
    @GeckoAuthority(codes = {"GECKO_WINELABELS_AL_CIRCULATION_EDIT"})
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_WINELABELS_AL_CIRCULATION_DELETE"})
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_WINELABELS_AL_CIRCULATION_NEW", "GECKO_WINELABELS_AL_CIRCULATION_EDIT"})
	public R save(@RequestBody AlCirculation entity) {
        R r = new R();
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_WINELABELS_AL_CIRCULATION_EXPORT"})
