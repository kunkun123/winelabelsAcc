...
import org.gecko.framework.manager.entity.MgCamera;
import org.gecko.framework.manager.service.MgCameraService;
 ...
 */
@Controller
@RequestMapping("/manager/mgcamera")
public class MgCameraController extends BaseController<MgCamera> {

	@Autowired
	public MgCameraService service;
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_MANAGER_MG_CAMERA_QUERY"})
	public Page<MgCamera> list(Page<MgCamera> page) {
        MgCamera param = new MgCamera();
...
param.setCameraip("%" + page.getSearch() + "%");
param.setSellerid("%" + page.getSearch() + "%");
param.setLineid("%" + page.getSearch() + "%");
...
//param.setStatus("%" + page.getSearch() + "%");
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_MANAGER_MG_CAMERA_VIEW"})
	public R get(@PathVariable String id) {
		MgCamera mgcamera = service.get(id);
		return R.ok().put(mgcamera);
	}

    @RequestMapping("/edit")
    @ResponseBody
    @GeckoAuthority(codes = {"GECKO_MANAGER_MG_CAMERA_EDIT"})
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_MANAGER_MG_CAMERA_DELETE"})
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_MANAGER_MG_CAMERA_NEW", "GECKO_MANAGER_MG_CAMERA_EDIT"})
	public R save(@RequestBody MgCamera entity) {
        R r = new R();
	...
	@ResponseBody
	@GeckoAuthority(codes = {"GECKO_MANAGER_MG_CAMERA_EXPORT"})
